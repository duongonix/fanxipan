...
const $derived = (v) => v;
const $dericved = (v) => v;
const __fanxipan_effect_defs = [{ deps: ["count"], run: () => console.log("count changed:", count, "double:", double) }, { deps: ["items"], run: () => console.log("items:", items.length, "done:", doneCount, "pending:", pendingCount) }];
const $effect = () => {};
...
const onclick = () => count++;
const onSave = (payload) => payload;
const addItem = () => query.trim() && (items = items.concat([{ id: nextId, text: query.trim(), done: false }]), nextId = nextId + 1, query = "");
const toggleItem = (id) => (items = items.map((x) => (x.id === id ? { id: x.id, text: x.text, done: !x.done } : x)));
const removeDone = () => (items = items.filter((x) => !x.done));
...
export function create(target, ctx) {
  ...
  const cmp_props_5 = { "name": name, "click": (event) => { onclick(event); if (ctx.notifyMany) ctx.notifyMany(["count", "double"]); }, "onSave": onSave };
 ...
 return frag; };
  ...
  const text_29 = document.createTextNode(" | (count * 2): ");
  el_25.appendChild(text_29);
  const text_30 = document.createTextNode(String((count * 2)));
  el_25.appendChild(text_30);
  ctx.subscribeExpr(["count"], () => { text_30.data = String((count * 2)); });
  ...
  const text_33 = document.createTextNode(String((items.filter((x) => x.done).length)));
  el_31.appendChild(text_33);
  ctx.subscribeExpr(["items"], () => { text_33.data = String((items.filter((x) => x.done).length)); });
  const text_34 = document.createTextNode(" | pending: ");
  el_31.appendChild(text_34);
  const text_35 = document.createTextNode(String((items.length - (items.filter((x) => x.done).length))));
  el_31.appendChild(text_35);
  ctx.subscribeExpr(["doneCount", "items"], () => { text_35.data = String((items.length - (items.filter((x) => x.done).length))); });
...
export const __fanxipanGraph = [["count", "derived:(count * 2)"], ["count", "effect:effect_1"], ["derived:(items.filter((x) => x.done).length)", "effect:effect_2"], ["derived:(count * 2)", "effect:effect_1"], ["derived:(items.length - (items.filter((x) => x.done).length))", "effect:effect_2"], ["doneCount", "derived:(items.length - (items.filter((x) => x.done).length))"], ["items", "derived:(items.filter((x) => x.done).length)"], ["items", "derived:(items.length - (items.filter((x) => x.done).length))"], ["items", "effect:effect_2"]];
